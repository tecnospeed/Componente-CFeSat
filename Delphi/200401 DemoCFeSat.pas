...
begin
   ...
   edtUltimoID.Text := ObterConteudo(aux, 85);
end;
...
begin
  ...
  try
    ...
    _dataSet.Campo('CNPJ_B11').Value := '00611455000143'; 	//CNPJ Software House 
    _dataSet.Campo('signAC_B12').Value := SignAC;			//Assinatura do Aplicativo Comercial 
    _dataSet.Campo('numeroCaixa_B14').Value := '123';		//Número do Caixa ao qual o SAT está conectado 

	//Identificação do Emitente do Cupom Fiscal eletrônico 
    _dataSet.Campo('CNPJ_C02').Value := '11111111111111';	//CNPJ do emitente 
    _dataSet.Campo('IE_C12').Value := '111111111111';		//IE 
    _dataSet.Campo('IM_C13').Value := '123123';				//Inscrição Municipal 
    _dataSet.Campo('cRegTribISSQN_C15').Value := '';		//Regime Especialde Tributação do ISSQN 
    _dataSet.Campo('indRatISSQN_C16').Value := 'N';			//Indicador de rateio do Desconto sobre subtotal entre itens sujeitos à tributação pelo ISSQN. 

	//Identificação do Destinatário do Cupom Fiscal eletrônco 
    _dataSet.Campo('CNPJ_E02').Value := '10575716000170';	//CNPJ do destinatário 	
    _dataSet.Campo('CPF_E03').Value := '';					//CPF do destinatário 
    _dataSet.Campo('xNome_E04').Value := 'Fazenda';			//Razão Social ou Nome do destinatário 

	// Identificação do Local de Entrega - Informar apenas no caso de entrega da mercadoria em domicílio 
    _dataSet.Campo('xLgr_G02').Value := '';					//Logradouro 
    _dataSet.Campo('nro_G03').Value := '';					//Número 
    _dataSet.Campo('xCpl_G04').Value := '';					//Complemento 
    _dataSet.Campo('xBairro_G05').Value := '';				//Bairro 
    _dataSet.Campo('xMun_G06').Value := '';					//município 
    _dataSet.Campo('UF_G07').Value := '';					// UF 

	//- Detalhamento de Produtos e Serviços 
    _dataSet.IncluirItem;
    _dataSet.Campo('nItem_H02').Value := '1';				//Número do item 
    _dataSet.Campo('cProd_I02').Value := '0001';			//Código do produto ou serviço, interno do contribuinte 
    _dataSet.Campo('cEAN_I03').Value := '0012345678905';	//GTIN (Global Trade Item Number) do produto, antigo código EAN ou código de barras 
															//não informar o conteúdo da TAG em caso de o produto não possuir este código. 
    _dataSet.Campo('xProd_I04').Value := 'Descrição do produto ou serviço';
															//????'Trib ICMS Integral Aliquota 10.00% - PIS e COFINS cod 04 - Orig 0';
    _dataSet.Campo('NCM_I05').Value := '47061000';			//Código NCM com 8 dígitos ou 2 dígitos (gênero) 
    _dataSet.Campo('CFOP_I06').Value := '5001';				//Código Fiscal de Operações e Prestações 
    _dataSet.Campo('uCom_I07').Value := 'kg';				//Unidade de comercialização / (combustíveis) - medida da codificação de produtos do  
